...
The example uses the included ZBar WASM polyfill. The [installation guide](./install) lets you switch to the compatible ZXing WASM option.

### Barcode format
`BarcodeScanner` scans `qr_code` by default. The `formats` option uses the [standard format names](https://developer.mozilla.org/en-US/docs/Web/API/Barcode_Detection_API#supported_barcode_formats) defined by the Barcode Detection API:
...
<Callout type="warning" emoji="⚠️">
  Available formats depend on the browser or polyfill BarcodeDetector implementation. Query `BarcodeDetector.getSupportedFormats()` at runtime, and see the [installation options](./install) for the ZBar and ZXing differences.
</Callout>
