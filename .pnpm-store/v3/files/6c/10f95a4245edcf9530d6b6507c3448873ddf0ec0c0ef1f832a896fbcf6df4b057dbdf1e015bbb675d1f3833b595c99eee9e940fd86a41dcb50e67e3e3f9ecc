...
var group = function group(routes) {
  ...
  routes.map(route => {
    ...
    if (!groupPath) {
      ...
      const isEntryMd = new RegExp(`^(index|readme)(\\.(${this.options.locales.map(([name]) => name).join('|')}))?$`, 'i').test(parsed.name); // only process nested route
  ...
  routes.forEach(route => {
    ...
    if (((_route$meta$group4 = route.meta.group) === null || _route$meta$group4 === void 0 ? void 0 : _route$meta$group4.path) && !route.meta.group.title) {
      ...
      route.meta.group.title = userCustomGroupTitles[route.meta.group.path]; // fallback use directory name
      ...
      if (!route.meta.group.title) {
        ...
        route.meta.group.title = // use second dir as group title
        ((_dirs$shift = dirs.shift()) === null || _dirs$shift === void 0 ? void 0 : _dirs$shift.replace(/^[a-z]/, s => s.toUpperCase())) || ( // then use nav title
        (_route$meta$nav5 = route.meta.nav) === null || _route$meta$nav5 === void 0 ? void 0 : _route$meta$nav5.title) || // fallback group title if there only has group path
        route.meta.group.path // discard nav prefix path or locale prefix path
        .replace(((_route$meta$nav6 = route.meta.nav) === null || _route$meta$nav6 === void 0 ? void 0 : _route$meta$nav6.path) || `/${route.meta.locale || ''}`, '') // discard start slash
