...
function getRouteComponentDirs(component, ctx) {
  ...
  const clearComponentPath = (0, _utils().winPath)(_path().default.join(component)); // find include path for current component path

  const entryIncludePath = clearIncludes.find(item => clearComponentPath.startsWith(item)); // extract component directories and split
  ...
  if (this.options.mode === 'site') {
    ...
    routes.forEach(route => {
      ...
      if (((_route$meta$nav3 = route.meta.nav) === null || _route$meta$nav3 === void 0 ? void 0 : _route$meta$nav3.path) && !route.meta.nav.title) {
        ...
        (_getRouteComponentDir = getRouteComponentDirs(route.component, this).shift()) === null || _getRouteComponentDir === void 0 ? void 0 : _getRouteComponentDir.replace(/^[a-z]/, s => s.toUpperCase())) || // fallback nav title by nav path
        // discard locale prefix
        (0, _locale.discardLocalePrefix)(route.meta.nav.path, route.meta.locale) // discard start slash
