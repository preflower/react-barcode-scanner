...
var _default = api => {
  ...
  const generateDemosFile = api.utils.lodash.debounce( /*#__PURE__*/_asyncToGenerator(function* () {
    ...
    const items = Object.keys(demos).map(uuid => {
      const componentName = demos[uuid].previewerProps.componentName; // collect component related module (react component & source code) into one chunk

      const chunkName = componentName && `demos_${[...componentName] // reverse component name to avoid some special component (such as Advertisement) be blocked by ADBlock when dynamic loading
      .reverse().join('')}` || 'demos_no_comp';
      const itemHoistImports = {};
      let demoComponent = demos[uuid].component; // replace to dynamic component for await import component

      demoComponent = (0, _utils2.decodeImportRequireWithAutoDynamic)(demoComponent, chunkName); // hoist all raw code import statements
  ...
  api.onGenerateFiles(generateDemosFile); // register demo detections
  ...
  api.modifyRoutes( /*#__PURE__*/function () {
    ...
    `;
      ...
      prependRoutes[0].wrappers = [// builtin outer layout for initialize context (.umi/dumi/layout.tsx)
