  ...
  return (ast, vFile) => {
    (0, _unistUtilVisit().default)(ast, 'element', (node, i, parent) => {
      ...
      if ((0, _hastUtilIsElement().default)(node, 'a') && (0, _hastUtilHasProperty().default)(node, 'href')) {
        ...
        let properties = Object.keys(node.properties).filter(prop => !['href'].includes(prop)).map(prop => `${prop}="${node.properties[prop]}"`).join(' '); // compatible with normal markdown link
