...
var _default = api => {
  ...
  api.chainWebpack( /*#__PURE__*/function () {
    var _ref = _asyncToGenerator(function* (memo) {
      const theme = yield (0, _loader.default)(); // set alias for dumi theme api

      memo.resolve.alias.set('dumi/theme', _path().default.join(__dirname, '../../theme')); // set alias for builtin default theme

      memo.resolve.alias.set('dumi-theme-default', _path().default.dirname(require.resolve('dumi-theme-default/package.json'))); // compile theme path for npm linked theme
